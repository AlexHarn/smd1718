...
# -------------------------------- Functions ----------------------------------
# All functions to prepare the data and evaluating the models
# -----------------------------------------------------------------------------
def rmse(y_true, y_pred):
    """
    The error metric. Rooted mean squared error of the logarithm of the sale
    prices. Assumes the input is already the logarithm.

    Parameters
    ----------
    y_true: logarithm of the true sale prices
    y_pred: logarithm of the predicted sale prices
...
def prepare(train_df, test_df):
    """
    Prepares the data for training. This also includes feature engineering.
    All of this has been copied from
    ...
    Returns
    -------
    Train dataframe, labels of the training data and test dataframe
    """
    ...
    # For imputing missing values: fill in missing LotFrontage values by the
    ...
    def munge(df):
        ...
        all_df["Fence"] = df["Fence"].map({None: 0, "MnWw": 1, "GdWo": 2,
                                           "MnPrv": 3, "GdPrv":
                                           4}).astype(int)
        ...
        # If YearRemodAdd != YearBuilt, then a remodeling took place at some
        # point.
        all_df["Remodeled"] = \
            (all_df["YearRemodAdd"] != all_df["YearBuilt"]) * 1

        # Did a remodeling happen in the year the house was sold?
        all_df["RecentRemodel"] = \
            (all_df["YearRemodAdd"] == all_df["YrSold"]) * 1
        ...
        # we still use them for later selecting the best features
        ...
        all_df["HighSeason"] = df["MoSold"].replace(
            {1: 0, 2: 0, 3: 0, 4: 1, 5: 1, 6: 1, 7: 1, 8: 0, 9: 0, 10: 0, 11:
             0, 12: 0})

        all_df["NewerDwelling"] = df["MSSubClass"].replace(
            {20: 1, 30: 0, 40: 0, 45: 0, 50: 0, 60: 1, 70: 0, 75: 0, 80: 0, 85:
        ...
        all_df["BoughtOffPlan"] = df.SaleCondition.replace(
            {"Abnorml": 0, "Alloca": 0, "AdjLand": 0, "Family": 0,
             "Normal": 0, "Partial": 1})
        ...
        all_df["SeasonSold"] = all_df["MoSold"].map({12: 0, 1: 0, 2: 0, 3: 1,
                                                     4: 1, 5: 1, 6: 2, 7: 2, 8:
                                                     2, 9: 3, 10: 3, 11:
                                                     3}).astype(int)
        ...
        # train_df["SalePrice"].groupby(train_df["Neighborhood"])
        # .median().sort_values()
        neighborhood_map = {
            "MeadowV": 0,  # 88000
            "IDOTRR": 1,   # 103000
    ...
    # Copy NeighborhoodBin into a temporary DataFrame because we want to use
    # the unscaled version later on (to one-hot encode it).
    neighborhood_bin_train = pd.DataFrame(index=train_df.index)
    neighborhood_bin_train["NeighborhoodBin"] = \
        train_df_munged["NeighborhoodBin"]
    neighborhood_bin_test = \
        pd.DataFrame(index=test_df.index)
    neighborhood_bin_test["NeighborhoodBin"] = \
        test_df_munged["NeighborhoodBin"]

    ###########################################################################

    numeric_features = train_df_munged.dtypes[train_df_munged.dtypes !=
                                              "object"].index
    ...
    def onehot(onehot_df, df, column_name, fill_na, drop_name):
        ...
        dummies = pd.get_dummies(onehot_df[column_name], prefix="_" +
                                 column_name)
    ...
    def munge_onehot(df):
        ...
        year_map = pd.concat(pd.Series("YearBin" + str(i+1),
                                       index=range(1871+i*20, 1891+i*20)) for i
                             in range(0, 7))
    ...
    return (train_df_munged, label_df, test_df_munged)


def evaluate_model(X, y, model, seed, n_splits, epochs, batch_size):
    """
    Evaluates the given model using the Keras wrapper for sklearn for k-fold
    cross validation.

    Parameters
    ----------
    X: training data to use
    y: training data labels
    model: the keras model to evaluate
    seed: random seed to use for consistency
    n_splits: number of splits for cross validation

    Returns
    -------
    Mean of the error and the error standard devivation
    """
    np.random.seed(seed)
    estimator = KerasRegressor(build_fn=model, epochs=epochs,
                               batch_size=batch_size, verbose=4)
    kfold = KFold(n_splits=n_splits, random_state=seed)
    # return the root of the results since we train with MSE but want to
    # evaluate using RMSE, also flip the sign again because cross_val_score
    # returns score not loss which in this case is the negative MSE
    results = np.sqrt(-cross_val_score(estimator, X, y, cv=kfold, verbose=2))
    return (results.mean(), results.std())


def show_learning(model, X, y, epochs=500, batch_size=200):
    """
    Train the choosen model with simple 0.2 validation_split to to get a
    rough idea of the training progress

    Parameters
    ----------
    model: the model to trian
    X: data to trian on
...
# ---------------------------------- Models -----------------------------------
#              The different models are defined in this section
# -----------------------------------------------------------------------------
def minimalistic_model(input_dim):
    """
    Returns a minimalistic model for testing and reference. One hidden fully
    connected layer with relu activation.
    10-fold cross validation: (0.17559779167983725, 0.028554750574823401)
    Submission score: 0.20233
    """
    model = Sequential()
...
def model_0(input_dim, dropout_ratio=0.2):
    """
    Deep Neural Network architecture with relu, Dropout and Batch Normalization
    after every layer. 3 Fully connected layers with input_dim neurons and 5
    not fully connected layers with half the amount of neurons of the previous
    layer
    """

    model = Sequential()

    # Input Dropout and Normalization
    model.add(Dropout(dropout_ratio, input_shape=(input_dim,)))
    model.add(BatchNormalization())

    # Fully connected layers
    for i in range(0, 3):
        model.add(Dense(input_dim, kernel_initializer="normal",
                        activation="relu"))
        model.add(Dropout(dropout_ratio))
        model.add(BatchNormalization())

    # More fully connected layers with less neurons than the previous ones
    if input_dim >= 200:
    ...
    model.add(Dropout(dropout_ratio))
    model.add(BatchNormalization())
    # for some reason two layers with 50 neurons improved performance
    model.add(Dense(50, kernel_initializer="normal", activation="relu"))
    model.add(Dropout(dropout_ratio))
    model.add(BatchNormalization())
...
def model_xlm(input_dim, layers=5, dropout_ratio=0.2):
    """
    Extreme Learning machine deep neural network architecture with relu,
    Dropout and Batch Normalization after every layer.
    Worse than model_0
    """
    if input_dim < 200:
        raise ValueError("Not enough features for this model!")
...
if __name__ == "__main__":
    ...
    print("Original feature count:", train_df.shape[1])

    # Prepare the data (credits: humananalog)
    X, y, X_submission = prepare(train_df, test_df)
    X = X.values
    X_submission = X_submission.values
    y = y.values
    print("Prepared data with", X.shape[1], "features")

    # Use xgb for feature selection, parameters from humananalog:
    # https://www.kaggle.com/humananalog/xgboost-lasso/
    ...
    regr.fit(X, y)
    y_pred_xgb = regr.predict(X_submission)
    # xgb.plot_importance(regr)
    # plt.show()

    # Find out for how many of the best features (according to xgb)
    # the DNN model performs best by cross validation
    thresholds = np.sort(regr.feature_importances_)
    scores = []
    # There is a RAM leak somewhere here so the loop will crash after some
    # time... So check for every threshold if the score has already been
    # computed
    known_thresholds, _, _ = np.loadtxt("feature_scores.csv", unpack=True,
                                        delimiter=",")
    # Sdjust the following line depending on which areas look interesting
    # and need more points
    thresholds = thresholds[(thresholds >= 0.00158) & (thresholds <= 0.00186)]
    """
    for threshold in np.random.choice(thresholds, size=10):
        if threshold in known_thresholds:
            print("Threshold already known, continuing...")
            continue
        print("Evaluating with threshold", threshold)
        selection = SelectFromModel(regr, threshold=threshold,
                                    prefit=True)
        X_sel = selection.transform(X)

        # Evaluate the model with cross validation on the training data
        scores.append(evaluate_model(X_sel, y, lambda input_dim=X_sel.shape[1]:
                                     model_0(input_dim), seed=1234, n_splits=5,
                                     epochs=230, batch_size=200))
        with open("feature_scores.csv", "a") as myfile:
            myfile.write("{0},{1},{2}\n".format(threshold, scores[-1][0],
                                                scores[-1][1]))
        print("Score", scores[-1])
    """

    # We choose
    threshold = 0.00162974
    # Transform the data with the choosen threshold
    selection = SelectFromModel(regr, threshold=threshold,
                                prefit=True)
    X_sel = selection.transform(X)
    X_submission = selection.transform(X_submission)
    print(X_sel.shape[1], "features left after feature selection")

    print("Evaluating model...")
    # show_learning(model_0(X_sel.shape[1]), X_sel, y)
    # Evaluate the model with cross validation on the training data
    print(evaluate_model(X_sel, y, lambda input_dim=X_sel.shape[1]:
                         model_0(input_dim), seed=4321, n_splits=10,
                         epochs=230, batch_size=200))

    print("Evaluating XGB...")
    np.random.seed(4321)
    kfold = KFold(n_splits=10, random_state=4321)
    model = xgb.XGBRegressor(colsample_bytree=0.2, gamma=0.0,
                             learning_rate=0.01, max_depth=4,
                             min_child_weight=1.5, n_estimators=7200,
                             reg_alpha=0.9, reg_lambda=0.6, subsample=0.2,
                             seed=42, silent=1)
    results = np.sqrt(-cross_val_score(model, X, y, cv=kfold, verbose=2,
                                       scoring="neg_mean_squared_error"))
    print(results.mean(), results.std())

    # Train the model without validation_split and create the submission file
    print("Training DNN for submission...")
    model = model_0(X_sel.shape[1])
    model.fit(X_sel, y, batch_size=200, epochs=300, verbose=2,
              validation_split=0)
    y_pred = np.exp(model.predict(X_submission))

    pred_df = pd.DataFrame(y_pred, index=test_df["Id"],
                           columns=["SalePrice"])
    pred_df.to_csv("submission_0.csv", header=True, index_label='Id')

    print("Creating XGB submission...")
    pred_df = pd.DataFrame(y_pred_xgb, index=test_df["Id"],
                           columns=["SalePrice"])
